...
const loanOut = function(amount) {
  return new Promise((resolve, reject) => {
    let loanedOutAmount = 0;
    if (creditLimit === 0) {
      return reject(`Error: not enough credit limit!`);
    }
    if (creditLimit < amount) {
      loanedOutAmount = amount - creditLimit;
      creditLimit = 0;

    } else {
      loanedOutAmount = amount;
      creditLimit -= amount;
    }
    return resolve(loanedOutAmount);
  });
};
